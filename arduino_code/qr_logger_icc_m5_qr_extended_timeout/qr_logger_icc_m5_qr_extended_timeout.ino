...
const unsigned long FS_RECOVERY_WINDOW_MS = 10000;
const unsigned long FS_FORMAT_HOLD_MS = 2000;
...
bool confirmFilesystemFormat() {
  printLine("LittleFS recovery: release, then hold the front button for 2 seconds within 10 seconds to format storage");
  M5.Display.fillScreen(COLOR_BG);
  M5.Display.setTextSize(1);
  M5.Display.setTextColor(COLOR_BAD, COLOR_BG);
  ...
  M5.Display.print("RELEASE, THEN HOLD");
  M5.Display.setCursor(8, 58);
  M5.Display.print("BUTTON FOR 2 SEC");
  M5.Display.setTextColor(COLOR_WARN, COLOR_BG);
  M5.Display.setCursor(8, 82);
  M5.Display.print("RELEASE = KEEP DATA");

  unsigned long windowStartedAt = millis();
  unsigned long holdStartedAt = 0;
  M5.update();
  bool confirmationArmed = !M5.BtnA.isPressed();
  while (millis() - windowStartedAt < FS_RECOVERY_WINDOW_MS) {
    M5.update();
    if (!confirmationArmed) {
      if (!M5.BtnA.isPressed()) confirmationArmed = true;
    } else if (M5.BtnA.isPressed()) {
      if (holdStartedAt == 0) holdStartedAt = millis();
      unsigned long heldFor = millis() - holdStartedAt;
      M5.Display.fillRect(8, 104, 112, 10, COLOR_BG);
      M5.Display.setTextColor(COLOR_ACCENT, COLOR_BG);
      M5.Display.setCursor(8, 104);
      M5.Display.printf("HOLD %lu/2 SEC", min(heldFor / 1000 + 1, 2UL));
      if (heldFor >= FS_FORMAT_HOLD_MS) return true;
    } else {
      holdStartedAt = 0;
    }
    delay(20);
...
void setStorageIssue(const char* reason) {
  storageFailureReason = reason;
  Serial.println(String("Storage issue: ") + reason);
}

void disableStorage(const char* reason) {
  storageReady = false;
  setStorageIssue(reason);
}
...
String readQrFrame() {
  ...
  // M5UnitQRCodeUART reports bytes currently available, not complete frames.
  // Finish only after the UART stays quiet, so a partial id is never accepted.
...
bool countJsonFilesChecked(const String& directoryPath, size_t& count) {
  count = 0;
  if (!storageReady) return false;
...
size_t countJsonFiles(const String& directoryPath) {
  size_t count = 0;
  countJsonFilesChecked(directoryPath, count);
  return count;
}
...
bool initializeStorage() {
  ...
  if (!preferences.begin("qrlogger", false)) {
    disableStorage("FS NVS");
    return false;
  }
  bool mounted = LittleFS.begin(false);

  if (!mounted && confirmFilesystemFormat()) {
    printLine("Physical confirmation received; formatting LittleFS");
    if (!LittleFS.format()) {
      preferences.end();
      disableStorage("FS FORMAT");
      return false;
    }
  ...
  if (!mounted) {
    preferences.end();
    disableStorage("FS MOUNT");
    printLine("LittleFS remains unavailable; storage was not formatted without physical confirmation");
    return false;
  }

  if ((!LittleFS.exists("/queue") && !LittleFS.mkdir("/queue")) ||
      (!LittleFS.exists("/profiles") && !LittleFS.mkdir("/profiles")) ||
      (!LittleFS.exists("/failed") && !LittleFS.mkdir("/failed"))) {
    preferences.end();
    disableStorage("FS DIR");
    printLine("LittleFS directory creation failed");
    return false;
  }
  ...
  if (!countPendingScans(currentPending)) {
    ...
    disableStorage("FS QUEUE");
    return false;
  }
  pendingScanCount = currentPending;
  size_t currentFailed = 0;
  if (!countJsonFilesChecked("/failed", currentFailed)) {
    disableStorage("FS FAILED");
    return false;
  }
  failedScanCount = currentFailed;
...
bool enqueueScan(const String& decodedData, String& scanId) {
  if (!storageReady) return false;
  if (pendingScanCount >= MAX_PENDING_SCANS) {
    setStorageIssue("Q FULL");
    return false;
  }
  if (!ensureQueueWriteSpace()) {
    setStorageIssue("FS SPACE");
    return false;
  }
...
bool connectWiFi(bool showUi = true) {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    printLine("WiFi timeout; status=" + String(static_cast<int>(WiFi.status())));
    if (showUi) {
...
void processPendingQueueOnce() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    printLine("Uploader: WiFi offline; starting reconnect");
    ...
    printLine("Uploader: WiFi reconnected");
  }
...
void setup() {
  ...
  else disableStorage("FS SYNC");
  displayStatus = "BOOT";
  uploadStatus = storageReady ? (pendingScanCount > 0 ? "Q:" + String(pendingScanCount) + " PENDING" : "STARTING") : storageFailureReason;
  drawScreen();
  startupTone();
  ...
  displayStatus = storageReady ? "READY" : storageFailureReason;
  uploadStatus = storageReady ? "SCAN QR" : "NOT SAVED";
  drawScreen();
  readyTone();
  ...
  if (data.length() > 0) {
    ...
    if (!validQr) {
      ...
      drawScreen();
        ...
        Serial.printf("Valid QR accepted; storage=%s pending=%u\n", storageReady ? "ready" : "unavailable", static_cast<unsigned int>(pendingScanCount));
        String cachedName;
        ...
        if (lockStorage()) {
          ...
          printLine("Scan not persisted: storage lock unavailable");
        }
        ...
        if (persisted) {
          ...
          printLine("Scan not persisted: " + (storageFailureReason.length() ? storageFailureReason : "unknown storage error"));
          displayStatus = storageFailureReason.length() ? storageFailureReason : "FS FAIL";
